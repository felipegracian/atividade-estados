...
const getListaDeEstados = function () {
   ...
   estadosCidades.estados.forEach(function (ufDosestados) {
      arrayListaDeEstados.push(ufDosestados.sigla)
      jSonUf.quantidade++
   })
...
const getDadosEstado = (siglaEstado) => {
   ...
   estadosCidades.estados.forEach(function (informacoesDoEstado) {
      if (informacoesDoEstado.sigla == siglaEstado.toUpperCase()) {
         ...
         dadosEstado.regiao = informacoesDoEstado.regiao
         status = true
      }
   })
...
const getCapitalEstado = (siglaEstado) => {
   let dadosEstado = { uf: '', descricao: '', capital: '' }
   let status = false
...
const getEstadosRegiao = (regiaoDesejada) => {
   ...
   let regiao = regiaoDesejada.toUpperCase()
   ...
   estadosCidades.estados.forEach(function (estadosRegiao) {
      if (estadosRegiao.regiao.toUpperCase() == regiao) {
         let listaSiglaEDescricao = {}
         listaSiglaEDescricao.uf = estadosRegiao.sigla
         listaSiglaEDescricao.descricao = estadosRegiao.nome
         arrayListaDeEstados.push(listaSiglaEDescricao)
         status = true
      }
   })

   if (status == true) {
      estadosDeCadaRegiao.regiao = regiao
...
const getCapitalPais = function () {
   ...
   estadosCidades.estados.forEach(function (mostrarCapital) {
      if (mostrarCapital.capital_pais != undefined) {
         let capitaisJSon = {}
         capitaisJSon.capital_atual = mostrarCapital.capital_pais.capital
         capitaisJSon.uf = mostrarCapital.sigla
         capitaisJSon.descricao = mostrarCapital.nome
         capitaisJSon.capital = mostrarCapital.capital
         capitaisJSon.regiao = mostrarCapital.regiao
         capitaisJSon.capital_pais_ano_inicio = mostrarCapital.capital_pais.ano_inicio
         capitaisJSon.capital_pais_ano_termino = mostrarCapital.capital_pais.ano_fim

         arrayCapitais.push(capitaisJSon)
   ...
   if (status == true) {
      JSoninicial.capitais = arrayCapitais
      return JSoninicial
   } else {
      return status
   }
...
const getCidades = (siglaEstado) => {
   let jsonCidadesDoEstado = { uf: '', descricao: '', quantidade_cidades: 0 }
   let arrayCidades = []
   let status = false
   ...
   estadosCidades.estados.forEach(function (listaDascidades) {
      if (listaDascidades.sigla == siglaEstado.toUpperCase()) {
         ...
         listaDascidades.cidades.forEach(function(colocarNoarray){
            arrayCidades.push(colocarNoarray.nome)
            jsonCidadesDoEstado.quantidade_cidades++
         })
   ...
   if(status == true){
      jsonCidadesDoEstado.cidades = arrayCidades
      return jsonCidadesDoEstado
   } else{
      return status
   }
